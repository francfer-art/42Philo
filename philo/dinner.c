...
/*   Updated: 2024/03/31 18:23:25 by francfer         ###   ########.fr       */
/*                                                                            */
/******************************************************************************/
...
static void eat(t_philo *philo)
{
    ...
    if (philo->table->limits_of_meals > 0 && philo->meals_counter == philo->table->limits_of_meals)
...
void    dinner_start(t_table *table)
{
    ...
    else
    {
        while (++i < table->philo_numbers)
            safe_thread_handler(&table->philos[i].philo_thread_id, dinner_simulation, &table->philos[i], CREATE);
    }
